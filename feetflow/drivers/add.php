...
$errors = [];
$data = [
    'full_name' => '', 'license_category' => 'B', 'license_expiry' => '',
    'safety_score' => '100', 'phone' => '', 'status' => 'Off Duty',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $db = getDB();
        $stmt = $db->prepare("INSERT INTO drivers (full_name, license_category, license_expiry, safety_score, phone, status)
            VALUES (:fn, :lc, :le, :ss, :ph, :st)");
        $stmt->execute([
            ':fn' => $data['full_name'], ':lc' => $data['license_category'],
            ':le' => $data['license_expiry'], ':ss' => $data['safety_score'],
            ':ph' => $data['phone'], ':st' => $data['status'],
        ]);
        setFlash('success', 'Driver added successfully!');
...
<div class="page-header">
    <h1><i class="bi bi-person-badge"></i>Add Driver</h1>
        ...
        <form method="POST" class="ff-form">
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" class="form-control" name="license_expiry" value="<?= sanitize($data['license_expiry']) ?>" required>
                </div>
                <div class="col-md-4">
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-ff"><i class="bi bi-check-lg me-1"></i>Save Driver</button>
            </div>
        </form>
    </div>
</div>
