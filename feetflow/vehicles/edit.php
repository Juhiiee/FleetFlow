...
$stmt->execute([':id' => $id]);
$data = $stmt->fetch();
if (!$data) { setFlash('danger', 'Vehicle not found.'); redirect(baseUrl() . '/vehicles/'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Unique license plate (exclude current)
    if (empty($errors)) {
        $stmt = $db->prepare("SELECT COUNT(*) FROM vehicles WHERE license_plate = :lp AND id != :id");
        $stmt->execute([':lp' => $data['license_plate'], ':id' => $id]);
        if ($stmt->fetchColumn() > 0) $errors[] = 'License plate already exists.';
    }

    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE vehicles SET vehicle_name=:vn, license_plate=:lp, vehicle_type=:vt,
            max_load_capacity=:ml, odometer=:od, acquisition_cost=:ac, region=:rg, status=:st WHERE id=:id");
        $stmt->execute([
            ...
            ':rg' => $data['region'], ':st' => $data['status'], ':id' => $id,
        ]);
...
<div class="page-header">
    <h1><i class="bi bi-truck"></i>Edit Vehicle</h1>
        ...
        <form method="POST" class="ff-form">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" name="region" value="<?= sanitize($data['region']) ?>">
                </div>
                <div class="col-md-4">
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-ff"><i class="bi bi-check-lg me-1"></i>Update Vehicle</button>
            </div>
        </form>
    </div>
</div>
